	...
	public sealed class SoftDeleteRule : ColumnRule
	{
		...
		public SoftDeleteRule(string columnName, object deletedValue, OperationTypes appliesWhen = OperationTypes.SelectOrDelete) : base(columnName, appliesWhen)
		{
			switch (appliesWhen)
			{
				...
				default:
					throw new ArgumentOutOfRangeException(nameof(appliesWhen), appliesWhen, "appliesWhen may only be Select or Delete");
			}
		...
		/// <returns></returns>
		public override object GenerateValue(object? argumentValue, object? userValue, object? currentValue) => DeletedValue;
	}
}
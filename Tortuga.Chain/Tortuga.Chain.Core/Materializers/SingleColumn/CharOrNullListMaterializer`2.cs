...
internal sealed class CharOrNullListMaterializer<TCommand, TParameter> : SingleColumnMaterializer<TCommand, TParameter, List<char?>> where TCommand : DbCommand
	where TParameter : DbParameter
{
	...
	/// </summary>
	/// <returns></returns>
	public override List<char?> Execute(object? state = null)
	{
					...
					for (var i = 0; i < columnCount; i++)
					{
						if (!reader.IsDBNull(i))
						{
							if (reader.GetFieldType(i) == typeof(string))
							{
								...
								result.Add(temp.Length > 1 ? temp[0] : default);
							}
							else
								result.Add(reader.GetChar(i));
						}
	...
	/// <returns></returns>
	public override async Task<List<char?>> ExecuteAsync(CancellationToken cancellationToken, object? state = null)
	{
		var result = new List<char?>();

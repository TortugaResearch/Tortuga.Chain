	...
	where TObject : class
{
	...
	public ILink<long> AsCountApproximate(string columnName)
	{
		...
		AggregationColumns.Add(new($"APPROX_COUNT_DISTINCT({column.QuotedSqlName})", "RowCount"));
	...
	public ILink<long> AsCountApproximate()
	{
		...
		AggregationColumns.Add(new($"APPROX_COUNT_DISTINCT({primaryKeys.Single().QuotedSqlName})", "RowCount"));
